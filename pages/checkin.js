...
export default function CheckInPage() {
  ...
  useEffect(() => {
    if (searchRef.current && selectedEvent && unlocked)
  ...
  const fetchStats = useCallback(async (eventId) => {
    try {
      ...
      setStats({
        total_attendees: parseInt(d.total_attendees) || 0,
        checked_in_count: parseInt(d.checked_in_count) || 0,
      });
    } catch {}
  }, []);
  ...
  useSocket(selectedEvent?.id, {
    ...
    onFinished: () => {
      setSelectedEvent((prev) =>
        prev ? { ...prev, is_finished: true } : prev,
      );
    },
    onRestarted: () => {
      setSelectedEvent((prev) =>
        prev ? { ...prev, is_finished: false } : prev,
      );
    },
  });
  ...
  const handleSearch = (value) => {
    ...
    searchTimeout.current = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await attendeesApi.getAll(selectedEvent.id, {
          search: value,
        });
  ...
  const handleCheckIn = async (attendee) => {
    try {
      ...
      if (err.response?.status === 409) {
        const alreadyChecked = err.response.data.data;
        setLastAction({ attendee: alreadyChecked, type: "already" });
        // Format the time client-side to respect the user's local timezone
        const checkedTime = alreadyChecked?.checked_in_at
          ? ` ${t.checkin_at} ${formatTime(alreadyChecked.checked_in_at)}`
          : "";
        toast.error(
          `${alreadyChecked?.name || attendee.name} ${t.checkin_already}${checkedTime}`,
        );
      } else {
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                setUnlocked(false);
                setStats({ total_attendees: 0, checked_in_count: 0 });
              }}
            >
              {t.checkin_change}
            </button>
              ...
              <div className="events-grid" style={{ maxWidth: "860px" }}>
                {events.map((event) => (
                  <div
                    ...
                    style={{ opacity: event.is_finished ? 0.85 : 1 }}
                  >
                    <div
                      style={{
                        display: "flex",
                    ...
                    <div className="event-card-stats">
                      <div className="event-card-stat">
                        <div className="event-card-stat-value">
                      ...
                      <div className="event-card-stat">
                        <div className="event-card-stat-value accent">
            ...
            <div>
              ...
              >
                  ...
                  <div>
                    ...
                    >
                      ...
                      {selectedEvent.date && selectedEvent.time
                        ? ` · ${selectedEvent.time.substring(0, 5)}`
                        : ""}
                ...
                <div
                  ...
                  style={{
                    marginBottom: selectedEvent?.is_finished ? "6px" : "10px",
                  }}
                >
                  <div
              ...
              {selectedEvent?.is_finished && (
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {t.checkin_event_finished}
                    </div>
                    <div
                      style={{ fontSize: "12px", color: "var(--text-muted)" }}
                    >
                      {t.checkin_event_finished_desc}
                    </div>
                  </div>
                </div>
              )}
              ...
              {lastAction && (
                <div
                  className={`checkin-result ${lastAction.type === "checkin" ? "success" : "warning"}`}
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {lastAction.attendee?.name}
                    </div>
                    {lastAction.attendee?.phone_number && (
                      <div
                        style={{
                          fontSize: "12px",
                          color: "var(--text-muted)",
                    ...
                    {lastAction.attendee?.checked_in_at && (
                      ...
                      >
                        {t.checkin_at}{" "}
                        {formatTime(lastAction.attendee.checked_in_at)}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="card">
                <div className="card-body">
                  <div
                    ...
                    style={{ marginBottom: results.length > 0 ? "14px" : 0 }}
                  >
                    <label className="form-label">
                      {t.checkin_search_label}
                    </label>
